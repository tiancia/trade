...
public class GeminiClient {
    ...
    public String generateText(GeminiGenerateReq req) {
        ...
        GeminiGenerateResp resp = parseContent(body);
    ...
    private GeminiGenerateResp parseContent(String body) {
        try {
            return objectMapper.readValue(body, GeminiGenerateResp.class);
        } catch (Exception e) {
            throw new AiResponseParseException("Parse Gemini response error: " + e.getMessage(), body, e);
        }
    }
    ...
    private String extractText(GeminiGenerateResp resp, String rawResponse) {
        if (resp == null || resp.getCandidates() == null || resp.getCandidates().isEmpty()) {
            throw new AiResponseParseException("Gemini response has no candidates", rawResponse);
        }
