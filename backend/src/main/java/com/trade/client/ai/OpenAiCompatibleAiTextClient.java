...
public class OpenAiCompatibleAiTextClient implements AiTextClient {
    ...
    public String generateJson(String prompt) {
        ...
        try {
            ...
            throw new AiResponseParseException(e.getMessage(), rawResponse, e);
        } catch (Exception e) {
            throw new AiResponseParseException(
                    "Parse OpenAI-compatible response error: " + e.getMessage(),
                    rawResponse,
                    e
            );
        }
    ...
    private String extractAssistantContent(String rawResponse) throws Exception {
        ...
        if (!choices.isArray() || choices.isEmpty()) {
            throw new AiResponseParseException("OpenAI-compatible response has no choices", rawResponse);
        }
