...
class AiPolymarketDecisionParserTest {
    ...
    void parsesBuyDecision() {
        ...
        assertEquals(new BigDecimal("0.5"), decision.getWinProbability());
        assertEquals(new BigDecimal("0.7"), decision.getConfidence());
    }
    ...
    void extractsJsonFromMarkdownFence() {
        AiPolymarketDecision decision = parser.parse("""
                ```json
                {"action":"HOLD","reason":"not enough edge","limitPrice":0,"maxSpendUsdc":0,"winProbability":0,"confidence":0,"estimatedEdge":0}
                ```
                """);
    ...
    @Test
    void acceptsLegacyEstimatedProbabilityField() {
        AiPolymarketDecision decision = parser.parse("""
                {"action":"BUY","reason":"legacy probability","outcome":"Yes","tokenId":"456","limitPrice":0.42,"maxSpendUsdc":5,"confidence":0.7,"estimatedProbability":0.52,"estimatedEdge":0.10}
                """);

        assertEquals(PolymarketAction.BUY, decision.getAction());
        assertEquals(new BigDecimal("0.52"), decision.getWinProbability());
        assertEquals(new BigDecimal("0.52"), decision.getEstimatedProbability());
    }
}
