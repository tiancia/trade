...
| 多实例领导权 | `trade_trading_leadership_enabled`、`trade_trading_leadership_active`、`trade_trading_leadership_operations_total` | 每个资金集群应恰好一个 leader；outcome、trigger |
...
- 已启用租约但集群无 leader、出现多个 leader、租约刷新失败；
- Redis 热行情缓存降级；
- K 线 SSE 更新被背压丢弃。
