...
def main():
    ...
    errors = validation_errors(payload, env, script_path, require_secrets=not args.print_only)
    if errors:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Debug harness for tools/polymarket/polymarket_place_order.py"
    )
    parser.add_argument(
        "--confirm-live-order",
        action="store_true",
...
def validation_errors(payload, env, script_path, require_secrets=True):
    errors = []
